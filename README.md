...
- **Cashflow at a glance** — three-number dashboard card: **Bank balance** (sum of latest balance per account), **Outstanding** (still-to-leave-the-bank commitments for the cycle), **Free now** (what's actually free to spend). Outstanding is **forecast-adjusted**: lines with a Forecast type commit their projected landing spend instead of the full budgeted remainder — a food line budgeted at R2 800 but projected to land at R2 700 releases R100 to Free now, while a line burning hot reserves the expected overshoot. Expand for a per-account breakdown with staleness indicators (red if >14 days since last sync, amber if >7) and a per-line table showing where each projection lands.
